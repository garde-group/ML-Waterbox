...
! This program is not perfect and there either exists a bug in this code or in my simulation
...
program data_format
    ...
    side = 4 ! Side length of box
    nuse = 10 ! Number of frames to use
    pnum = 2180 ! Number of particles
    na = 3
    nskip = 0
    ...
    if (ret == 1) then
        c1 = 0
            do while (ret == 1 .and. nframe < (nskip + nuse))
                    ...
                    coord, prec, ret)
					do k = 1, size(coord), 9
						! Go through the coordinates to find the order parameter for each one
                        points = 0
                        dist = 10
                        c1 = c1 + 1
						...
						do m = 1, size(coord), 9
							! Go through all the other coordinates to find the nearest 4
                            if (m == k) cycle
							...
							! Keeping track of the nearest four distances and points
						...
						! Copy the near points over
                        do m = 1, size(points)
                            results(c1, m+3) = points(m)
                        end do
                        sigma = 0
						do i = 1, 9, 3
							! This is the calculation of the order paramter, using the law of cosines
							! Here's a good website for more information http://www.phy.cuhk.edu.hk/opus/comments_2014/lcc_pre.pdf
